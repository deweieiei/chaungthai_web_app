...
(function () {
  ...
  const elIcon = document.getElementById('phone-icon');
  const elTitle = document.getElementById('phone-title');
  const elDisplay = document.getElementById('phone-display');
  ...
  function render() {
    ...
    const verified = !!u.user_phone_verified_at;
    elDisplay.textContent = phone || '(ยังไม่ตั้งเบอร์)';
    elIcon.textContent = verified ? '✅' : '📱';
    elTitle.textContent = verified ? 'ยืนยันเบอร์โทรแล้ว' : 'เบอร์โทรของคุณ';
    ...
    if (!phone) {
      elContent.innerHTML = `
        <p class="text-muted text-center">กรุณาตั้งเบอร์โทรในโปรไฟล์ก่อนยืนยัน</p>
        <a href="/profile/edit" class="btn btn--primary btn--block" style="margin-top: var(--space-md);">ไปกรอกเบอร์โทร</a>`;
      return;
    }
  ...
  async function requestOtp(e) {
    ...
    try {
      const res = await Api.post('/auth/verify-phone/request');
      UI.toast('ส่งรหัส OTP ไปยังเบอร์โทรแล้ว');
      startCountdown(60);
      if (res.otp_code) {
  ...
  async function confirmOtp(e) {
    ...
    UI.setBtnLoading(btn, true);
    try {
      await Api.post('/auth/verify-phone/confirm', { otp_code: otp });
      await refreshUser();
      UI.toast('ยืนยันเบอร์โทรสำเร็จ', 'success');
